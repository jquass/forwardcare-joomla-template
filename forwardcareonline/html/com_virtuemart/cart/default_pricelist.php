...
<div class="billto-shipto">
	<div class="billto floatleft">
		...
		</a>		<?php						if (!class_exists ('VmHtml')) {					require(JPATH_VM_ADMINISTRATOR . DS . 'helpers' . DS . 'html.php');				}							echo JText::_ ('<br />Same Billing/Shipping Address');			echo VmHtml::checkbox ('STsameAsBTjs', $this->cart->STsameAsBT) . '<br />';		?>		
	</div>
	
	<div class="shipto floatleft" id="output-shipto-display">
...
<tr valign="top" class="sectiontableentry<?php echo $i ?>">
	<td align="left">
		...
		<?php } ?>				<!-- NOTE 1 name  -->
		<?php 		$qurl = $prow->url;		$qname = $prow->product_name;		$qfields = $prow->customfields;		$qsku = $prow->product_sku;				if(!$qsku)		{						preg_match_all("/\(.*?\)/",$qfields,$qmatches);						$qsku =  $qmatches[0][1] ;						preg_match_all("/\[.*?\]/",$qfields,$qmatches);			$qfields = $qmatches[0][0];					} else {				}				$qname = $qsku . ' ' . $qname . ' ' . $qfields;				echo JHTML::link ($qurl, $qname); 						?>		
